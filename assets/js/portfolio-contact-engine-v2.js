(() => {
  ...
  const contactEngineArrivalDuration = 620;
  ...
  const createContactEngine = (origin, arriving = false) => {
    const root = document.createElement('div');
    root.className = `contact-engine-v2${arriving ? ' is-arriving' : ''}`;
    root.setAttribute('aria-hidden', 'true');
    root.innerHTML = `
      <span class="contact-engine-v2__veil"></span>
      <span class="contact-engine-v2__line"></span>
      <span class="contact-engine-v2__core"></span>`;
    root.style.setProperty('--contact-origin-x', `${origin.x}px`);
    root.style.setProperty('--contact-origin-y', `${origin.y}px`);
    document.documentElement.appendChild(root);
    return {
      root,
      veil: root.querySelector('.contact-engine-v2__veil'),
      line: root.querySelector('.contact-engine-v2__line'),
      core: root.querySelector('.contact-engine-v2__core'),
      origin
    };
  };

  const playContactPhase = async (element, keyframes, options, finalStyles = {}) => {
    if (!element) return;
  ...
  const runContactDeparture = async (engine) => {
    const { root, veil, line, core, origin } = engine;
    const originTransform = `translate3d(0,${origin.y}px,0)`;
    const centreTransform = `translate3d(0,${window.innerHeight * .5}px,0)`;
    root.style.opacity = '1';

    await playContactPhase(core, [
      { opacity: 0, transform: 'translate(-50%,-50%) rotate(45deg) scale(.45)' },
      { opacity: 1, transform: 'translate(-50%,-50%) rotate(45deg) scale(1)', offset: .42 },
      { opacity: 0, transform: 'translate(-50%,-50%) rotate(0) scale(2.15,.08)' }
    ], { duration: 180, easing: 'cubic-bezier(.16,1,.3,1)' });

    await playContactPhase(line, [
      { opacity: 0, transform: `${originTransform} scaleX(0)` },
      { opacity: .82, transform: `${originTransform} scaleX(1)` }
    ...
    await playContactPhase(line, [
      { opacity: .82, transform: `${originTransform} scaleX(1)` },
      { opacity: .76, transform: `${centreTransform} scaleX(1)` }
    ], { duration: 260, easing: 'cubic-bezier(.16,1,.3,1)' }, {
      opacity: '.76', transform: `${centreTransform} scaleX(1)`
    });

    const lineLeft = `${origin.x}px`;
    const lineWidth = `${Math.max(window.innerWidth - origin.x, 1)}px`;
    await Promise.all([
      playContactPhase(line, [
        { left: lineLeft, width: lineWidth, opacity: .76, transform: `${centreTransform} scaleX(1)` },
        { left: '0px', width: `${window.innerWidth}px`, opacity: 0, transform: `${centreTransform} scaleX(1.02)` }
      ], { duration: 360, easing: 'cubic-bezier(.55,0,.3,1)' }),
      playContactPhase(veil, [
        { clipPath: 'inset(0 0 0 100%)' },
        { clipPath: 'inset(0)' }
      ], { duration: 360, easing: 'cubic-bezier(.72,0,.24,1)' }),
      playContactPhase(document.body, [
        { opacity: 1, transform: 'translate3d(0,0,0)' },
        { opacity: .2, transform: 'translate3d(-8vw,0,0)' }
      ], { duration: 360, easing: 'cubic-bezier(.55,0,.3,1)' }, {
        opacity: '.2', transform: 'translate3d(-8vw,0,0)'
      })
    ]);
  ...
  const resetNavigationState = () => {
    ...
    document.querySelectorAll('.archive-transition, .contact-engine-v2').forEach((transition) => transition.remove());
  ...
  const showPageArrival = async () => {
    ...
    if (pageTransition?.type === 'contact-engine-v2' && document.body.classList.contains('info-contact-page')) {
      const engine = createContactEngine({ x: window.innerWidth, y: window.innerHeight * .5 }, true);
      engine.root.style.opacity = '1';
      document.body.style.opacity = '.2';
      document.body.style.transform = 'translate3d(8vw,0,0)';
      await nextPaint();
      document.documentElement.classList.remove('page-transition-boot');
      await Promise.all([
        playContactPhase(engine.veil, [
          { clipPath: 'inset(0)' },
          { clipPath: 'inset(0 100% 0 0)' }
        ], { duration: contactEngineArrivalDuration, easing: 'cubic-bezier(.72,0,.24,1)' }),
        playContactPhase(document.body, [
          { opacity: .2, transform: 'translate3d(8vw,0,0)' },
          { opacity: 1, transform: 'translate3d(0,0,0)' }
        ], { duration: contactEngineArrivalDuration, easing: 'cubic-bezier(.16,1,.3,1)' }, {
          opacity: '1', transform: 'translate3d(0,0,0)'
        })
      ]);
      engine.root.remove();
  ...
  document.querySelectorAll('[data-contact-entry]').forEach((link) => {
    ...
    link.addEventListener('click', async (event) => {
      ...
      if (navigationInProgress) return;
      navigationInProgress = true;
      target.searchParams.set(contactTransitionQueryKey, 'engine-v2');
      ...
      const engine = createContactEngine(from);
      freezeNavigationSource();
